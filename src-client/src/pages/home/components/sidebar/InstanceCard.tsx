...
import { useDeleteInstanceMutation, useFindAllInstanceQuery } from '~/hooks/api/useInstance';

interface InstanceCardProps {
  data: InstanceDto;
}

export default function InstanceCard({ data }: InstanceCardProps) {
  const navigate = useNavigate();

  const deleteInstance = useDeleteInstanceMutation();
  const { refetch } = useFindAllInstanceQuery();

  return (
    <div
      className="relative w-full h-[40%] group overflow-hidden"
      style={{
        backgroundImage: `url(${import.meta.env.MODE !== 'development' ? modpackLogo : 'https://i.imgur.com/4b1k0aH.png'})`,
    ...
    >
      ...
      <div className="absolute left-0 right-0 bottom-0 w-full p-2 bg-base-300/60 space-y-3 transform transition-transform duration-300 translate-y-[calc(100%-40px)] group-hover:translate-y-0 group-focus:translate-y-0">
        ...
        <div className="join flex">
          <button className="btn btn-primary join-item flex-1" onClick={() => navigate(`/instances/${data.id}`)}>
            Customize
          </button>
        </div>
      </div>
