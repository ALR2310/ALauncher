...
export async function createInstance(instance: InstanceMeta) {
...
export async function updateInstance(currentSlug: string, patch: Partial<InstanceMeta>) {
  ...
  const nextMeta: InstanceMeta = { ...metaFromInstance(full), ...patch };
  if (patch.slug) nextMeta.slug = patch.slug;
  ...
  mf.instances[idx] = nextMeta;
  await writeManifest(mf);

  return { success: true, meta: nextMeta, filePath: nextPath };
}

export async function deleteInstance(slug: string) {
  const filePath = instancePath(slug);
  ...
  return { success: true, message: 'Modpack đã được xoá.', slug };
}

export async function getInstance(slug?: string) {
  if (slug) {
    const filePath = instancePath(slug);
    const full = await readJSON<InstanceType | null>(filePath, null as any);
    return full ?? null;
  } else {
    const manifest: ManifestType = await readManifest();
    return manifest.instances ?? [];
  }
}

export async function addMods(slug: string, mods: Mod | Mod[]) {
  const filePath = instancePath(slug);
  const full = await readJSON<InstanceType | null>(filePath, null as any);
  if (!full) return { success: false, message: 'Modpack không tồn tại.' };
...
export async function removeMods(slug: string, ids: string | string[]) {
  const filePath = instancePath(slug);
  const full = await readJSON<InstanceType | null>(filePath, null as any);
  if (!full) return { success: false, message: 'Modpack không tồn tại.' };
